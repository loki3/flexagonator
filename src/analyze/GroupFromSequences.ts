namespace Flexagonator {
  ...
  export function getGroupFromSequences(
    ...
    if (typeof (rows) === 'string') {
      return { reason: 'incomplete', minimalPats, sequences: [rows] };
    }
   ...
   */
  function makeRows(fm: FlexagonManager, flexElements: FlexName[][]): number[][] | string {
    const rows: number[][] = [];
    fm.normalizeIds();
    ...
    for (const e1 of flexElements) {
      ...
      for (const e2 of flexElements) {
        ...
        if (index === -1) {
          ...
          return e1.map(e => e.fullName).join('') + ' ' + e2.map(e => e.fullName).join('');
        }
        row.push(index);
        fm.undoAll();
      }
