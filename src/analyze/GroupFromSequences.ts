namespace Flexagonator {
  ...
  export interface GroupError {
    readonly reason:
    | 'not-cyclic'        /** one or more sequences aren't cycles */
    | 'changes-structure' /** one or more sequences change the pat structure */
    | 'redundant'         /** the sequences are cyclic & structure preserving, but multiple sequences have same effect */
    | 'incomplete'        /** the sequences are cyclic & structure preserving, but incomplete */
    | 'bad-numpats'       /** numpats and/or the associated directions don't make a valid flexagon */
    | 'unsupported-flex'; /** one or more sequences contain a flex that's not supported on this flexagon */
    readonly sequences?: string[];  /** [optional] one or more sequences that caused the error */
  }
  export function isGroupError(result: any): result is GroupError {
    return result && (result as GroupError).reason !== undefined;
  }
  ...
  function changesStructure(flexagon: Flexagon, sequences: FlexName[][], morphs: Flexes): boolean {
   ...
   */
  function makeRows(fm: FlexagonManager, flexElements: FlexName[][]): number[][] | GroupError {
    const rows: number[][] = [];
    fm.normalizeIds();
