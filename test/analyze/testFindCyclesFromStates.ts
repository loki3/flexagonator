namespace Flexagonator {

  describe('FindCyclesFromStates', () => {
    const allFlexes6 = makeAllFlexes(6);
    // the 5 states of a tetra-hexaflexagon you can reach with P
    const states4_6 = [
      Flexagon.makeFromTree([[[1, 10], 7], 2, [[3, 11], 8], 4, [[5, 12], 9], 6]) as Flexagon,
      Flexagon.makeFromTree([[[-11, -3], 4], 8, [[-10, -1], 2], 7, [[-12, -5], 6], 9]) as Flexagon,
      Flexagon.makeFromTree([[[7, -6], 1], -12, [[9, -4], 5], -11, [[8, -2], 3], -10]) as Flexagon,
      Flexagon.makeFromTree([[-10, -1], [8, -2], [-11, -3], [9, -4], [-12, -5], [7, -6]]) as Flexagon,
      Flexagon.makeFromTree([[[4, -9], -11], 5, [[6, -7], -12], 1, [[2, -8], -10], 3]) as Flexagon,
    ];

    function findCyclesFromStates(
      states: Flexagon[], start: number, flexes: Flexes
    ): CycleInfo[] | FindCycleError {
      const finder = new FindCyclesFromStates(states, start, flexes);
      while (finder.checkNext()) { }
      const error = finder.getError();
      return error !== null ? error : finder.getCycles();
    }


    it('finds cycles', () => {
      const flexes: Flexes = { P: allFlexes6['P'], '>': allFlexes6['>'], '^': allFlexes6['^'] };
      const result = findCyclesFromStates(states4_6, 2, flexes);
      if (isGroupCycleError(result)) {
        fail('failed to find cycle');
        return;
      }

      expect(result.length).toBe(18);
      const sequences = result.map(c => `(${c.sequence})${c.cycleLength}`);
      expect(sequences[0]).toBe("(P P <<^)2");
      expect(sequences[3]).toBe("(P <^P <<^)6");
      expect(sequences[6]).toBe("(P >)9");
      expect(sequences[17]).toBe("(<^P ^)9");
    });

    it('complains if no shift flexes passed in', () => {
      const flexes: Flexes = { P: allFlexes6['P'], S: allFlexes6['S'] };
      const result = findCyclesFromStates(states4_6, 2, flexes);
      if (!isGroupCycleError(result)) {
        fail('failed to return error');
        return;
      }
      const expected: FindCycleError = { findCycleError: 'need definitions for > and ^' };
      expect(result.findCycleError).toBe(expected.findCycleError);
    });

    it('complains about invalid start', () => {
      const flexes: Flexes = { P: allFlexes6['P'], '>': allFlexes6['>'], '^': allFlexes6['^'] };
      const result = findCyclesFromStates(states4_6, 10, flexes);
      if (!isGroupCycleError(result)) {
        fail('failed to return error');
        return;
      }
      const expected: FindCycleError = { findCycleError: "invalid start" };
      expect(result.findCycleError).toBe(expected.findCycleError);
    });
  });
}
